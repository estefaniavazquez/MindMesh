...
def admin_creation():
    ...
    admin_id = create_admin("estefania")
    assert admin_id == 1

    admin_id_other = create_admin("maxyo")
    assert admin_id_other == 2
...
def user_creation():
    ...
    user_id = create_user("estefania")
    assert user_id == 1

    user_id_other = create_user("maxyo")
    assert user_id_other == 2
...
def knowledge_profile_and_learner_profile_attach_to_user():
    ...
    result_id = create_knowledge_profile(username, kp)
    assert result_id == 1
    ...
    result_id = create_learner_profile(username, lp)
    assert result_id == 1
